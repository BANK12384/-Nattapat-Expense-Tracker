...
3. คำแนะนำสำหรับการวางแผนออมเงินหรือลดรายจ่ายในเดือนถัดไป`;
  ...
  try {
    var data = JSON.parse(e.postData.contents);
    // ใส่ FOLDER ID ของ Google Drive คุณตรงนี้
    var folderId = "${driveFolderId || 'YOUR_GOOGLE_DRIVE_FOLDER_ID'}"; 
    var folder = DriveApp.getFolderById(folderId);
...
service cloud.firestore {
  match /databases/{database}/documents {
    // ป้องกันคนอื่นอ่านเขียนข้ามบัญชี 
    // อนุญาตเฉพาะ UID ตรงกับเจ้าของข้อมูล
    match /artifacts/{appId}/users/{userId}/{document=**} {
      allow read, write: if request.auth != null && request.auth.uid == userId;
    }
  }
              ...
              <form onSubmit={handleAddTransaction} className="space-y-4">
                ...
                {transType === 'TRANSFER' && (
                  <div className="p-3 bg-sky-950/50 border border-sky-800/50 rounded-xl text-xs text-sky-300 leading-relaxed">
                    💡 **โอนย้ายบัญชี:** ปรับยอดคงเหลือระหว่างสองบัญชีให้อัตโนมัติ โดย **ไม่นับ** เป็นรายจ่ายหรือรายรับจริง
                  </div>
                )}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-6">
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-3">
              ...
              <ol className="list-decimal list-inside text-xs text-slate-300 space-y-1.5 leading-relaxed pl-1">
                ...
                <li>ไปที่ <a href="https://script.google.com" target="_blank" rel="noreferrer" className="text-emerald-400 underline">script.google.com</a> กด **New Project**</li>
                <li>วางโค้ด <span className="font-mono text-emerald-300">Code.gs</span> แล้วเปลี่ยน `folderId` เป็น ID ของคุณ</li>
                <li>กด **Deploy** ➔ **New deployment** ➔ เลือกชนิด **Web app**</li>
                <li>ตั้งค่า **Who has access** เป็น <span className="font-semibold text-emerald-400">Anyone</span> แล้วกด **Deploy**</li>
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-2">
              ...
              <ul className="list-disc list-inside text-xs text-slate-400 space-y-1">
                ...
                <li><b className="text-slate-200">Hosting:</b> สามารถนำซอร์สโค้ดนี้ไปโฮสต์ฟรีบน Vercel, Netlify หรือ GitHub Pages ได้เลย</li>
              </ul>
            </div>
          </div>
        )}
